...
public abstract class EuclidActivity extends Activity {
    ...
    protected PermissionsActivity Per;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.Per = new PermissionsActivity(this);
        Per.setPermission();
    ...
    public void connectVPN() {
        if (isFirst) {
            Toast.makeText(this, "确认后请在点击一次", Toast.LENGTH_LONG).show();
    ...
    private void startVpn() {
        try {
            InputStream conf = getAssets().open("client.ovpn");// your own file in /assets/client.bin
    ...
    private void addOverlayListItem(Map<String, Object> item, View view) {
        ...
        Picasso.with(EuclidActivity.this).load(new File((String) getSDPath() + item.get(EuclidListAdapter.KEY_AVATAR)))
        ...
        Log.v("Test", "Array " + item.get(EuclidListAdapter.KEY_AVATAR));
    ...
    private ShapeDrawable buildAvatarCircleOverlay() {
        ...
        ShapeDrawable overlay = new ShapeDrawable(
                new RoundRectShape(null, Rf, new float[]{radius, radius, radius, radius, radius, radius, radius, radius})
        );
